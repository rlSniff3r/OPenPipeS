...
# --- Instalação Granular de Ferramentas ---
def get_go_env():
...
"""
    for rc_file in rc_files:
        try:
            with open(rc_file, "r") as f: content = f.read()
        except FileNotFoundError: content = ""
        if "OPENPIPES_DIR" not in content:
            with open(rc_file, "a") as f: f.write("\n" + config_block)

    symlinks = {
        "init-openpipes.sh": "init-openpipes", "openpipes_orchestrator.sh": "openpipes",
        "recon.sh": "recon", "nwrapper.sh": "nwrapper", "cria_Alvos_Obsidian.sh": "cria-alvos",
        "httpx-runner.sh": "httpx-runner", "katana-buster.sh": "katana-buster",
        ...
        "cria_Vulnerabilidades.sh": "cria-vulnerabilidades", "vuln-enricher.sh": "vuln-enricher",
        "screenshot-runner.sh": "screenshot-runner", "feroxbuster-runner.sh": "feroxbuster-runner",
        "katana-runner.sh": "katana-runner"
    }
    for src, link in symlinks.items():
        src_path = f"{OPENPIPES_SCRIPTS}/{src}"
        if os.path.exists(src_path): run_cmd(f"ln -sf '{src_path}' '{OPENPIPES_BIN}/{link}'")

    core_wrapper = f'#!/bin/bash\nsource "{HOME}/.openpipes/.venv-core/bin/activate"\npython "{OPENPIPES_DIR}/openpipes_core/cli.py" "$@"\ndeactivate\n'
    with open(f"{OPENPIPES_BIN}/openpipes-core", "w") as f: f.write(core_wrapper)
