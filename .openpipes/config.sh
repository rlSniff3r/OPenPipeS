...
NUCLEI_DIR="$proj_path/nuclei-output"
HTTPX_DIR="$proj_path/httpx-output"
KATANA_DIR="$proj_path/katana-output"
FEROX_DIR="$proj_path/ferox-output"
base_dir=$NMAP_DIR
...
export proj_name proj_dir proj_path obsdir
...
# FUNÇÕES AUXILIARES
ensure_dirs() {
    mkdir -p "$proj_path" "$NMAP_DIR" "$RECON_DIR" "$NUCLEI_DIR" \
             "$HTTPX_DIR" "$KATANA_DIR" "$FEROX_DIR" "$LOG_DIR"
}
